               ...
               unsigned short numObsTypes, int i)
{
   ...
   {
    if( (input.obsList[j].obsType == C1 || input.obsList[j].obsType == C2 ||     //Added by Marques, H. A.
         ...
         input.obsList[j].obsType == L5 || input.obsList[j].obsType == C5 ||   //Added by Marques, H. A.
         input.obsList[j].obsType == C5 || input.obsList[j].obsType == D5 ||
         input.obsList[j].obsType == S5 )
...
//    allowed range for the data element.  NULLs are typically all 9999s
   ...
   CounterobsType=0;       //Added by Marques, H. A
   ...
   enum { VersionRec,   PgmRunByRec,     CommentRec,     MarkerNameRec,
         ...
         if( L2_fac > 2 )
	 {
      ...
      else if ( inputRec.find( "# / TYPES OF OBSERV" )  != string::npos )
      {
         ...
         if(numObsTypes == 9999)  //Added by Marques, H. A.
         {
              temp = inputRec.substr( 0, 6 );
              if( getLong(temp, tempL) )
       ...
       /*  //commented Marques, H. A.  It is already being initialized in the class constructor
          for ( i = 0; i < MAXOBSTYPES; i++ )
         {
            obsTypeList[ i ] = NOOBS;
         }
        ...
        //CounterobsType - was added by Marques, H. A.
         ...
         while( nextObs >> obsType )
	 {
      ...
      /*   enum OBSTYPE { NOOBS = 0, L1 = 1, L2 = 2, C1 = 3, P1 = 4, P2 = 5,
             ...
             if ( obsType.find( "L1" ) != string::npos ) obsTypeList[ CounterobsType ] = L1;
        else if ( obsType.find( "L2" ) != string::npos ) obsTypeList[ CounterobsType ] = L2;
        else if ( obsType.find( "C1" ) != string::npos ) obsTypeList[ CounterobsType ] = C1;
        else if ( obsType.find( "P1" ) != string::npos ) obsTypeList[ CounterobsType ] = P1;
        else if ( obsType.find( "P2" ) != string::npos ) obsTypeList[ CounterobsType ] = P2;
        else if ( obsType.find( "C2" ) != string::npos ) obsTypeList[ CounterobsType ] = C2;
        else if ( obsType.find( "D1" ) != string::npos ) obsTypeList[ CounterobsType ] = D1;
        else if ( obsType.find( "D2" ) != string::npos ) obsTypeList[ CounterobsType ] = D2;
        else if ( obsType.find( "T1" ) != string::npos ) obsTypeList[ CounterobsType ] = T1;
        else if ( obsType.find( "T2" ) != string::npos ) obsTypeList[ CounterobsType ] = T2;
        else if ( obsType.find( "S1" ) != string::npos ) obsTypeList[ CounterobsType ] = S1;
        else if ( obsType.find( "S2" ) != string::npos ) obsTypeList[ CounterobsType ] = S2;
        else if ( obsType.find( "L5" ) != string::npos ) obsTypeList[ CounterobsType ] = L5;
...
bool RinexObsFile::validEventFlagRecord(string inputRec)
{
  ...
  string cols61thru80 = inputRec.substr(60,20);
  int tempUS;

  //********Added by Marques, H. A.*****************************************
    temp = inputRec.substr( 28, 1 );
    if( getLong(temp, tempL) )
      tempUS = static_cast< unsigned short >( tempL );
    if( tempUS == 4)    //header information follows
    {
        tempStream << "On line #" << getNumberLinesRead() << ":"
                   << endl << inputRec << endl
                   << "   header information follows: " << temp << "." << endl;
        appendToWarningMessages( tempStream.str() );

        return false;
    }

    if(cols61thru80.compare("COMMENT             ")==0)
    {
        tempStream << "On line #" << getNumberLinesRead() << ":"
                   << endl << inputRec << endl
