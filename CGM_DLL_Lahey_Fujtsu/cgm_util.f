	SUBROUTINE GEODCGEOMAG(DAY,UT,LAT,LON,MLAT,MLON,MLT,LER)
...
C
      ...
      !DEC$ ATTRIBUTES DLLEXPORT::GEODCGEOMAG
	...
	CHARACTER*50 PATH
	INTEGER LUN 
	REAL UT,LAT,LON,MLAT,MLON,MLT
	integer DAY,LER,I,J,K
      INCLUDE 'cgdb.inc'
c	DAY = 231
c	UT = 0
c	LAT = -22
...
c	LUN =1
      IF (LER.EQ.1) THEN
c	  PATH = 'C:\gpseq_BC_Ha\campo_geomagnetico\PIM\CGMDB\'

  	  CALL RDCGDB

	ELSE 
c	  OPEN(UNIT=2,FILE='CG.TXT',STATUS='UNKNOWN',FORM='FORMATTED')
c	 WRITE(2,100) (((CG(I,J,K),I=1,2),J=1,36),K=1,89)
c  100 FORMAT(0P8F6.1)
...
C	  DO LON=-175,175,5
	    ...
	    CALL GCXCGM(DAY,UT,LAT,LON,MLAT,MLON,MLT)
c          WRITE(*,*)DAY,UT,LAT,LON,MLAT,MLON
C		LON = LON + 5
...
C
       ...
       LUN = 1
c      CALL STRCCT(PATH,'cglalo.dat',32,FSPEC,I)

      OPEN(UNIT=LUN,FILE='CGLALO.DAT',STATUS='OLD',
...
C
      ...
      RETURN
  100 FORMAT(0P8F6.1)
  10	write(*,*) 'Error opening CGLATLO file in cgm_util.f'
      write(*,*)' '
c	pause
	stop
      END
...
C
      ...
      REAL PMAT(2),PMONG(2),QLAT,QLONG,RAD,RLAT(2),RLONG(2),SB,SC,SGN
