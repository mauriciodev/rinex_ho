...
C
       XLT0 = 90.-XLAT(J)
       TH1 = (CGMLT-XMLT0(J))*3.1415926/12.
       XCT1 = XLT0*COS(TH1)
...
C
C       There is an error
        PRINT *,' Index, lat,local time radius,discriminant = '
        PRINT *,CGMLT,J,XLAT(J),XMLT0(J),RADIUS(J),DISC
...
C        Otherwise , take the postitve one
        ...
        ELSE IF (SOL1 .GT. 0) THEN
         REGB(J)=SOL2
        ELSE
         PRINT *,' Index, lat,local time radius = '
         PRINT *,CGMLT,J,XLAT(J),XMLT0(J),RADIUS(J)
