...
//------------------------------------------------------------------------------------
double Class_TEC::Calc_STEC_PR(double CA,double P2,int prn)
{/*-------------------------------------------------------------------
...
double f1_quad = pow(freq1,2),
       f2_quad = pow(freq2,2),
       constante = (f1_quad*f2_quad)/(40.3*(f2_quad-f1_quad)),
       //values in meters
       Bsm = c*(p1p2[prn][0]*1.0e-9),
       Brm = c*(br*1.0e-9),
       Bp1c1 = c*(p1c1[prn][0]*1.0e-9);

double Stec,var_tec;

    Stec = constante*(  ( (CA+Bp1c1) - P2) - Brm - Bsm  );

    //Computing Standard Deviation of TEC
    double var_bsm = pow(p1p2[prn][1],2),
           var_brm = pow(sbr,2);

    var_tec = pow(constante,2)*(var_ca + var_p2 + var_brm + var_bsm  ); //(ele/m^2)
 ...
 int prn;
    ...
    while(!arqu1.eof())
    {
      ...
      if(i==0)
      {
        ...
        p1c1[prn][0]=atof(linha.substr(3,11).c_str());   //Read P1-C1
        p1c1[prn][1]=atof(linha.substr(15,12).c_str());  //Read P1-C1 RMS
      }
    ...
    while(!arqu2.eof())
    {
      ...
      {
        prn = atoi(linha.substr(1,2).c_str());
        p1p2[prn][0] = atof(linha.substr(3,11).c_str());  //Read P1-P2
        p1p2[prn][1] = atof(linha.substr(15,12).c_str()); //Read P1-P2 RMS
      }
...
int itemp;
    ...
    try{
            ...
            f1.getline(Line,82);
            line1=Line;
            if(line1.substr(60,21).compare("IONEX VERSION / TYPE") != 0)
            {
            ...
            do
            {
                  ...
                  if(line1.substr(60,21).compare("PGM / RUN BY / DATE ") == 0) // PROGRAM, AGENCY, DATE
                  {
                  ...
                  else if(line1.substr(60,21).compare("COMMENT             ") == 0)  //SKIP COMMENT LINES
                  {
                      RegFlag[2]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("DESCRIPTION         ") == 0)  //SKIP DESCRIPTION LINES
                  {
                      RegFlag[3]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("EPOCH OF FIRST MAP  ") == 0)  //EPOCH OF FIRST MAP
                  {
                  ...
                  else if(line1.substr(60,21).compare("EPOCH OF LAST MAP   ") == 0) //EPOCH OF LAST MAP
                  {
                      int IYEAR,IMONTH,IDAY,IHOUR,IMIN,ISEC;
                      sscanf(line1.c_str(),"%6d %6d %6d %6d %6d %6d",&IYEAR,&IMONTH,&IDAY,&IHOUR,&IMIN,&ISEC);
                      RegFlag[5]=1;
                  ...
                  else if(line1.substr(60,21).compare("INTERVAL            ") == 0)
                  {
                      sscanf(line1.c_str(),"%6d",&iint);
                      RegFlag[6]=1;
                      Interval=double(iint);
                  }//else if
                  else if(line1.substr(60,21).compare("# OF MAPS IN FILE   ") == 0)
                  {
                      sscanf(line1.c_str(),"%6d",&NMAP);
                      RegFlag[7]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("MAPPING FUNCTION    ") == 0)
                  {
                      MappFunction = line1.substr(2,4);
                     //sscanf(line1.c_str(),"%s",MappFunction.c_str());
                      RegFlag[8]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("ELEVATION CUTOFF    ") == 0)
                  {
                      sscanf(line1.c_str(),"%lf",&Elevation_Cuttof);
                      RegFlag[9]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("OBSERVABLES USED    ") == 0)
                  {
                      ObsUsed = line1.substr(0,60);
                      RegFlag[10]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("# OF STATIONS       ") == 0)
                  {
                      sscanf(line1.c_str(),"%6d",&itemp);
                      RegFlag[11]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("# OF SATELLITES     ") == 0)
                  {
                      sscanf(line1.c_str(),"%6d",&itemp);
                      RegFlag[12]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("BASE RADIUS         ") == 0)
                  {
                      sscanf(line1.c_str(),"%lf",&Base_Radius);
                      RegFlag[13]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("MAP DIMENSION       ") == 0)
                  {
                      sscanf(line1.c_str(),"%6d",&idim);
                      RegFlag[14]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("HGT1 / HGT2 / DHGT  ") == 0)
                  {
                      sscanf(line1.c_str(),"%lf %lf %lf",&Ini_Height,&End_Height,&Height_Incremet);
                      RegFlag[15]=1;
                 }//else if
                  else if(line1.substr(60,21).compare("LAT1 / LAT2 / DLAT  ") == 0)
                  {
                      sscanf(line1.c_str(),"%lf %lf %lf",&Ini_Lat,&End_Lat,&Lat_Increment);
                      RegFlag[16]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("LON1 / LON2 / DLON  ") == 0)
                  {
                      sscanf(line1.c_str(),"%lf %lf %lf",&Ini_Lon,&End_Lon,&Lon_Increment);
                      RegFlag[17]=1;
                  }//else if
                  else if(line1.substr(60,21).compare("EXPONENT            ") == 0) // EXPONENT
                  {
                      sscanf(line1.c_str(),"%d",&Iexp);
                      RegFlag[18]=1;
                  } //else if
                  else if(line1.substr(60,21).compare("START OF AUX DATA   ") == 0)  // AUXILIARY DATA BLOCK FOUND
                  {
                      ...
                      }while (line1.substr(60,21).compare("END OF AUX DATA     ") != 0);

                      RegFlag[20] = 1;
                  }//else


            }while( line1.substr(60,21).compare("END OF HEADER       ") != 0 &&
                    line1.substr(60,21).compare("END OF FILE         ") != 0);


            if(line1.substr(60,21).compare("END OF FILE         ") ==0)
            {
...
string line1;
     ...
     do     //LOOP OVER ALL DATA LINES
     {
          ...
          // LOOK FOR CURRENT KEYWORD AND STORE CORRESPONDING INFORMATION
          // ------------------------------------------------------------
          if(line1.substr(60,21).compare("START OF TEC MAP    ") == 0) //START OF TEC MAP
          {
              sscanf(line1.c_str(),"%6d",&IMAP);
              ITYP=0;
              NUMMAP[ITYP]++;
          }//if
          else if(line1.substr(60,21).compare("START OF RMS MAP    ") == 0)
          {
              Header_Flag = 1;
              break;
          }//else if
          else if(line1.substr(60,21).compare("START OF HEIGHT MAP ") == 0) //START OF HEIGHT MAP
          {
              if (ITYP==-1)
                sscanf(line1.c_str(),"%6d",&IMAP);
              ...
              do{     //SKIP HEIGHT MAP
                   ...
                   f1.getline(Line,82);
                   line1=Line;
                }while(line1.substr(60,21).compare("END OF HEIGHT MAP   ") != 0);

          }//else
          else if (line1.substr(60,21).compare("EPOCH OF CURRENT MAP") == 0)
          {
              int IYEAR,IMONTH,IDAY,IHOUR,IMIN,ISEC;
          ...
          else if (line1.substr(60,21).compare("EXPONENT            ") == 0)
          {
              //EXPONENT
               sscanf(line1.c_str(),"%6d",&Iexp);
          }
          else if (line1.substr(60,21).compare("COMMENT             ") == 0)
          {
              //COMMENT
          }
          else if (line1.substr(60,21).compare("                    ") == 0)
          {
              //SKIP BLANK LINES
          }
          else if (line1.substr(60,21).compare("LAT/LON1/LON2/DLON/H") == 0)
          {
              if (ITYP == -1)
              {
     ...
     }while(line1.substr(60,21).compare("END OF FILE         ") != 0);
...
bool time_gim = false;
    ...
    if(time_gim == false)  //It was not foud the MJD in the GIM
      return(false);
